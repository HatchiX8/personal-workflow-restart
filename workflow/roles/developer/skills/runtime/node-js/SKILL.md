---
name: node-js
description: Developer 的 Node.js Runtime 規則。當 project.config.json 的作用中 stack 宣告 runtimes 包含 node-js，或本次工作明確涉及 Node.js runtime、套件或建置工具鏈時使用。
---

# Developer Node.js Runtime Skill

只定義 Node.js runtime、非同步程序、模組與套件生態規則。Target、Language、Framework 與 Task 規則由其他槽位 Skills 補充。

---
name: frontend
description: Developer 的前端 Target 規則。當 project.config.json 的作用中 stack 宣告 target=frontend，或 repository evidence 明確顯示本次工作屬於前端時使用。
---

# Developer 前端 Target Skill

套用所有前端任務共通的架構、資料流、UI 與完成檢查規則。語言與 Framework 規則由其他槽位 Skills 補充。
...
- 若已選 Language Skill 要求靜態檢查，是否已執行對應指令。

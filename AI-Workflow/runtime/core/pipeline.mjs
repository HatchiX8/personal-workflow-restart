...
const hasHighConfidenceProvenance = (manifest, field, minimumConfidence) => {
  const confidence = provenanceConfidence(manifest, field);
  return confidence !== null && confidence >= minimumConfidence;
};

const normalizeRiskTrigger = (trigger, policy) => {
  const canonicalCandidate = trigger.startsWith('risk=') ? trigger.slice('risk='.length) : trigger;
  return policy.trigger_aliases?.[canonicalCandidate] ?? canonicalCandidate;
};
...
export function assessTaskRisk(manifest, policy = defaultTaskRiskPolicy, options = {}) {
  const minimumConfidence = options.minimumConfidence ?? 0.9;
  const hardTriggerIds = new Set(policy.hard_triggers ?? []);
  const triggers = new Set(
    (manifest.routing_triggers ?? []).map((trigger) => normalizeRiskTrigger(trigger, policy))
  );
  const moduleCount = manifest.modules?.length ?? 0;
  const includePaths = manifest.scope?.include_paths ?? [];
  const scopeHasHighConfidence = hasHighConfidenceProvenance(manifest, 'scope', minimumConfidence);
  const triggerHasHighConfidence = hasHighConfidenceProvenance(manifest, 'routing_triggers', minimumConfidence);
  const scopeDrivers = new Set();
  let scopeMode = 'unknown';
  if (
    manifest.scope?.change_source === 'full-project' ||
    triggers.has('full-project') ||
    triggers.has('scope=full-project')
  ) {
    scopeDrivers.add(manifest.scope?.change_source === 'full-project' ? 'scope' : 'routing_triggers');
    scopeMode = 'full-project';
  } else if (moduleCount > 1 || triggers.has('cross-module') || triggers.has('scope=cross-module')) {
    scopeDrivers.add(moduleCount > 1 ? 'modules' : 'routing_triggers');
    scopeMode = 'cross-module';
  } else if (moduleCount === 1 || triggers.has('scope=module')) {
    scopeDrivers.add(moduleCount === 1 ? 'modules' : 'routing_triggers');
    scopeMode = 'module';
  } else {
    scopeDrivers.add('scope');
    scopeDrivers.add('modules');
    if (
      (includePaths.length === 1 && scopeHasHighConfidence) ||
      (triggers.has('scope=file') && triggerHasHighConfidence)
    ) {
      scopeDrivers.clear();
      scopeDrivers.add(triggers.has('scope=file') ? 'routing_triggers' : 'scope');
      scopeMode = 'file';
    }
  }

  const hardTriggerSources = new Map();
  const addHardTrigger = (trigger, sourceFields) => {
    if (!trigger || !hardTriggerIds.has(trigger)) return;
    if (!hardTriggerSources.has(trigger)) hardTriggerSources.set(trigger, new Set());
    for (const sourceField of sourceFields) hardTriggerSources.get(trigger).add(sourceField);
  };
  for (const trigger of triggers) addHardTrigger(trigger, ['routing_triggers']);

  const derivedHardTriggers = policy.derived_hard_triggers ?? {};
  addHardTrigger(derivedHardTriggers.scope_mode?.[scopeMode], scopeDrivers);
  addHardTrigger(derivedHardTriggers.target_mode?.[manifest.target_mode], ['targets']);
  addHardTrigger(derivedHardTriggers.task_type?.[manifest.task_type], ['task_type']);
  const hardTriggers = [...hardTriggerSources.keys()].sort();

  const unresolved = [];
  if (manifest.status !== 'analyzed' || (manifest.unresolved ?? []).length > 0) {
    unresolved.push('task-manifest-not-fully-analyzed');
  }
  let level = policy.default_level ?? 2;
  const reasons = [];
  if (hardTriggers.length > 0) {
    level = 3;
    reasons.push(...hardTriggers.map((trigger) => `hard-trigger=${trigger}`));
  } else {
    const level1Policy = policy.level_1 ?? {};
    const level1 = (level1Policy.scope_modes ?? []).includes(scopeMode)
      && (manifest.targets ?? []).length === level1Policy.target_count
      && (level1Policy.task_types ?? []).includes(manifest.task_type)
      && manifest.target_mode === level1Policy.target_mode;
    if (level1) {
      level = 1;
  ...
  if (hardTriggers.length > 0) {
    for (const sources of hardTriggerSources.values()) {
      for (const source of sources) driverFields.add(source);
    }
  } else {
    for (const source of scopeDrivers) driverFields.add(source);
    driverFields.add('targets');
    driverFields.add('task_type');
    driverFields.add('routing_triggers');
  }
  const driverConfidences = [];
  for (const field of driverFields) {
    const confidence = provenanceConfidence(manifest, field);
    if (confidence === null) {
      unresolved.push(`risk-provenance-missing:${field}`);
      driverConfidences.push(0);
    } else {
      driverConfidences.push(confidence);
      if (confidence < minimumConfidence) unresolved.push(`risk-provenance-low-confidence:${field}`);
    }
  }
  const confidence = driverConfidences.length > 0 ? Math.min(...driverConfidences) : 0;
  return {
    schema_version: '1.0',
...
export function resolveExecutionProfile(manifest, taskRisk, role = null, skills = []) {
  ...
  const stagesByProfile = {
    lightweight: ['role-planner', 'rule-resolution', 'context-resolution', 'preflight', 'executor-adapter', 'role-entry'],
  ...
  const stages = stagesByProfile[profileId] ?? [];
  return {
    schema_version: '1.0',
    task_id: manifest.task_id,
    risk_level: taskRisk.level,
    profile_id: profileId,
    required_stages: blocked ? [] : stages,
    skipped_stages: blocked ? stages : [],
    reasons: [...new Set(reasons)],
    upward_escalation: true,

...
這項設定只用於讓 Developer、Review 與 Project Analyst 重用已存在的 Module Context，不是執行
Module Analyst 的前置條件。Project Analyst 只把 Module Context 當 optional 參考，缺少或不相容
不得阻擋。只有 Prompt 含完全相等的獨立一行 `角色：module-analyst` 才會啟動 Module Analyst；它會
以名稱為搜尋種子，直接從 Project Root 的 repository evidence 建立檔案範圍，不需要先設定
`module_registry`、alias 或每個檔案路徑，也不會載入既有 Module Context 代替本次探索。
